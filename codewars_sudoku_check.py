def done_or_not(board): #board[i][j]
  ...
  # ..checking rows first
    ...
    for index in range(0,9):
        print (board[index])
        lst = [] 
        for jndex in range(0,9):
            lst.append(board[index][jndex])
        lst.sort()
        if val_list != lst:
            return 'Try again!'
    for index in range(0,9):
        lst = []
        for jndex in range(0,9):
            lst.append(board[jndex][index])
        lst.sort()
        if val_list != lst:
            return 'Try again!'            
    return 'Finished!'
    
    cell = [[board[0][0],board[1][0],board[2][0],board[0][1],board[1][1],board[2][1],board[0][2],board[1][2],board[2][2]]
